...
export function requireMinimum(min: number, message?: string): Validator<number> {
    return v => (hasValue(v) && v < min)
        ? [message || `The value must not be less than ${min}`]
        : null;
}

export function requireMaximum(max: number, message?: string): Validator<number> {
    return v => (hasValue(v) && v > max)
        ? [message || `The value must not be more than ${max}`]
        : null;
}

export function requireBetween(min: number, max: number, message?: string): Validator<number> {
    return v => (hasValue(v) && (v < min || v > max))
        ? [message || `The value should be between ${min} and ${max}`]
        : null;
}

export function requireMinimumLength(min: number, message?: string): Validator<string | any[]> {
...
export function requireMaximumLength(max: number, message?: string): Validator<string | any[]> {
...
export function requireLengthBetween(min: number, max: number, message?: string): Validator<string | any[]> {
...
export function requirePattern(pattern: RegExp, message = "The answer provided is invalid"): Validator<string> {
